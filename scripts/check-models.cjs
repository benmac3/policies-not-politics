...
const data={};for(const name of ['observations','sources','countries','claims'])data[name]=JSON.parse(fs.readFileSync(path.join(root,'data',name+'.json')));
...
el('#country-search').value='Australia';el('#country-scope').value='all';vm.runInContext('countryTable()',ctx);assert(el('#country-table').innerHTML.includes('Not loaded'));
